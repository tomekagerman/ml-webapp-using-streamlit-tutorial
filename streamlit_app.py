...
if st.button("Predict Dropout Status"):
    # Using Neutral Averages for the other 13 hidden features
    input_data = np.array([
        22.4, 1.0, 0.7, 0.8, 2.5,  # Age, Gender, Eth, Edu, Income
        gpa,                       # User Input
        attendance,                # User Input
        0.6, 0.4, 1.2,             # Extra, Job, Self-study
        study_hours,               # User Input
        4.5, 0.8, 0.7,             # Failures, Scholarship, Internet
        stress,                    # User Input
        2.1, 0.6                   # Fin-Stress, Peer Influence
    ])
